<template>
	<view class="flex-col justify-start relative page">
		...
		<image class="image_5 pos_4" src="../../../images/page18/fa3babe67a5849c8174f1ef2cfde632c.png" />
		...
		<scroll-view :scroll-y="true" class="flex-col section_3 pos_10">
			<view class="flex-col list">
				<view class="flex-col justify-start list-item" v-for="(item, index) in list" :key="index">
					<view class="flex-col section_4">
						<text class="self-start font text_3">任务单号：{{item.taskSheetCode}}</text>
						<view class="mt-16 flex-row items-center self-stretch">
							...
							<view class="ml-8 flex-col">
								<view class="flex-row self-stretch">
									...
									<text class="ml-6 self-start font_2">项目名称：{{item.projectName}}</text>
								</view>
								<text class="self-start font_3">车牌号：{{item.carNumber}}</text>
								<text class="self-stretch font_3 text_4">发货日期：{{item.deliveryDate}}</text>
								<text class="self-stretch font_3">司机手机号：{{item.driverMobilePhone}}</text>
								<text class="self-start font_3">方量：{{item.concreteQuantity}}</text>
								<text class="self-start font_3">退回方量：{{item.returnConcreteQuantity}}</text>
								<text class="self-start font_3">楼号：{{item.buildingNumber}}</text>
								<text class="self-start font_3">状态：{{item.state}}</text>
							</view>
						</view>
					</view>
				</view>
