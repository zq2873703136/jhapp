<template>
	<view class="flex-col justify-start relative page">
		...
		<text class="text_2 pos_2">配合比查询</text>
			...
			<view class="flex-col list">
				<view class="flex-col justify-start list-item" v-for="(item, index) in list" :key="index">
					<view class="flex-col section_4">
						<text class="self-start font text_3">编号：{{item.code}}</text>
						<view class="mt-16 flex-row items-center self-stretch">
							...
							<view class="ml-8 flex-col">
								<view class="flex-row self-stretch">
									...
									<text class="ml-6 self-start font_2">强度等级：{{item.strengthGrade}}</text>
								</view>
								<text class="self-start font_3">施工坍落度：{{item.constructionSlump}}</text>
								<text class="self-stretch font_3 text_4">设计坍落度：{{item.designSlump}}</text>
								<text class="self-stretch font_3">结构部位：{{item.structuralPart}}</text>
							</view>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				const res = await mixProportionNoticeForm()
				console.log(res,'res');
				this.list = res.data
			}
		},
