<template>
	<view class="flex-col justify-start relative page">
		...
		<text class="text_2 pos_2">司机方量表</text>
			...
			<view class="flex-col list">
				<view class="flex-col justify-start list-item" v-for="(item, index) in list" :key="index">
					<view class="flex-col section_4">
						<text class="self-start font text_3">司机姓名：{{item.username}}</text>
						<view class="mt-16 flex-row items-center self-stretch">
							...
							<view class="ml-8 flex-col">
								<view class="flex-row self-stretch">
									...
									<text class="ml-6 self-start font_2">方量：{{item.concreteQuantity}}</text>
								</view>
								<text class="self-start font_3">车数：{{item.carCount}}</text>
								<text class="self-start font_3">公里数：{{item.distance}}</text>
								<text class="self-stretch font_3 text_4">运输工资：{{item.constructionPrice}}</text>
							</view>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				const res = await driverConcreteQuantityList({startDate:'2023-07-01',endDate:'2024-08-16'})
				console.log(res,'res');
				this.list = res.data;
			}
		},
