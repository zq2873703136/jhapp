<template>
	<view class="flex-col justify-start relative page">
		...
		<text class="text_2 pos_2">原材料统计</text>
		...
		<scroll-view :scroll-y="true" class="flex-col overflow section_3 pos_10">
			<view class="flex-col list">
				<view class="flex-col justify-start list-item" v-for="(item, index) in list" :key="index">
					<view class="flex-col section_4">
						<text class="self-start font text_3">材料名称：{{item.materialName}}</text>
						<view class="mt-16 flex-row items-center self-stretch">
							...
							<view class="ml-8 flex-col">
								<view class="flex-row self-stretch">
									...
									<text class="ml-6 self-start font_2">规格：{{item.materialSpec}}</text>
								</view>
								<text class="self-start font_3">单位：{{item.unit}}</text>
								<text class="self-stretch font_3 text_4">入库时间：{{item.inventoryEntryDate}}</text>
								<text class="self-stretch font_3">车辆名称：{{item.carName}}</text>
								<text class="self-stretch font_3">车牌号：{{item.carNumber}}</text>
								<text class="self-stretch font_3">进货单位：{{item.corpName}}</text>
								<text class="self-stretch font_3">称量毛重：{{item.weighingGrossWeight}}</text>
								<text class="self-stretch font_3">称量皮重：{{item.weighingTareWeight}}</text>
								<text class="self-stretch font_3">毛重：{{item.grossWeight}}</text>
								<text class="self-stretch font_3">皮重：{{item.tareWeight}}</text>
								<text class="self-stretch font_3">扣称：{{item.deductionScale}}</text>
								<text class="self-stretch font_3">净重：{{item.netWeight}}</text>
								<text class="self-stretch font_3">价格：{{item.constructionSite}}</text>
								<text class="self-stretch font_3">仓位：{{item.warehouseName}}</text>
								<text class="self-stretch font_3">状态：{{item.status}}</text>
								<text class="self-stretch font_3">材料员：{{item.materialsClerk}}</text>
							</view>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				const res = await materialForm({pageNum:1,pageSize:10})
				console.log(res,'res');
