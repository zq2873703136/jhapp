<template>
	<view class="flex-col justify-start relative page">
		...
		<text class="text_2 pos_2">车方量统计</text>
			...
			<view class="flex-col list">
				<view class="flex-col justify-start list-item" v-for="(item, index) in list" :key="index">
					<view class="flex-col section_4">
						<text class="self-start font text_3">车牌号：{{item.carNumber}}</text>
						<view class="mt-16 flex-row items-center self-stretch">
							...
							<view class="ml-8 flex-col">
								...
								<text class="self-stretch font_3 text_4">退转方量：{{item.returnConcreteQuantity}}</text>
							</view>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				const res = await vehicleConcreteQuantityList({startDate:'2023-07-01',endDate:'2024-08-16'})
				console.log(res,'res');
				this.list = res.data;
			}
		},
