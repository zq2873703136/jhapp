<template>
	<view class="flex-col justify-start relative page">
		...
		<text class="text_2 pos_2">原材料库存</text>
			...
			<view class="flex-col list">
				<view class="flex-col justify-start list-item" v-for="(item, index) in list" :key="index">
					<view class="flex-col section_4">
						<text class="self-start font text_3">仓位：{{item.inventoryCode}}</text>
						<view class="mt-16 flex-row items-center self-stretch">
							...
							<view class="ml-8 flex-col">
								<view class="flex-row self-stretch">
									...
									<text class="ml-6 self-start font_2">磅房材料：{{item.materialName}}</text>
								</view>
								<text class="self-start font_3">材料规格：{{item.materialSpec}}</text>
								<text class="self-stretch font_3 text_4">现有库存：{{item.inventoryCount}}</text>
								<text class="self-stretch font_3">材料单位：{{item.unit}}</text>
							</view>
						</view>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getList() {
				const res = await materialInventory({pageNum:1,pageSize:10})
				console.log(res,'res');
